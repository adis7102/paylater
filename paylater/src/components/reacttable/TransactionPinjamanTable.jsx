...
const { getAllPinjaman, searcher, updateStatusPinjaman, getPenarikan, searcherTransaction } = dashboardAction
...
const TransactionPinjamanTable = props => {
    ...
    const searchByDate = (startDate, endDate) => {
        props.searcherTransaction({type : 'penarikan', path : `riwayat_penarikan?start_date=${startDate}&end_date=${endDate}`})   
    }
        ...
        () => [
            ...
            },
            {
                ...
                Cell: props => (
                    <div className="react-action-class">
                        {
                            currencyFormatter.format(props.original.jumlah_penarikan, { code: 'IDR' })
                        }
                    </div>
                )
            },
            {
                ...
                Cell: props => (
                    <div className="react-action-class">
                        {
                            currencyFormatter.format(props.original.jumlah_penarikan - props.original.jumlah_dana_cair, { code: 'IDR' })
                        }
                    </div>
                )
            },
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            if (searchText !== null) {
                props.searcherTransaction({type : 'penarikan', path : `riwayat_penarikan?borrower_name=${searchText}`})
            }
        }, 800)
    ...
    const callListApi = (params) => {
        ...
        props.getPenarikan({ page: params.page })
                ...
                <div className="roe-card-style mtb-15">
                    <div className="roe-card-header module-header">
                        <div className="flex-1 font-weight-bold">
                            <span className="hash"># </span> PINJAMAN
                        </div>
                        ...
                        <div className="mr-1">
                            <Button
                                ...
                                onClick={() => props.getPenarikan({ page: '1' })}
                                style={{
                    ...
                    <div className="roe-card-body">
                        <ReactTable
                            ...
                            totalPage={props.penarikan.total_page}
                            // ini gapenting
                            // changeMethodFlag={changeMethodFlag}
                            // resetMethodFlag={() => setChangeMethodFlag(false)}
                        />
                    </div>
                    <ModalDownload modalDownload={modalDownload} closeModalDownload={closeModalDownload} path="api/notif/riwayat_penarikan" type="penarikan" />
                    <CustomToast
...
export default connect(
    mapStateToProps,
    mapDispatchToProps
)(withRouter(TransactionPinjamanTable));